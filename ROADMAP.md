...
- ✅ **News Sentry** — NSE corporate announcements on every held name → LLM triage (untrusted-content boundaries, JSON-mode, allowlist-validated fyi/look/decide) → `book_events` + Telegram. *Closes the Cupid gap.* Verified live on CUPID.NS (real filings correctly triaged, idempotent re-runs). RSS (Moneycontrol/ET) deferred — NSE announcements alone deliver the core win without three flaky scrapers.
- ✅ **Calendar** — F&O expiry from the Archivist's own captured data (NSE's `FininstrmActlXpryDt`, never a hardcoded "last Thursday" rule) + RBI MPC/Union Budget static dates → Watchtower flags. *Earnings + ex-dividend explicitly deferred — no confirmed free NSE source yet.*
- ✅ **India-native signal-family machinery** — `indiaBacktest.ts`: real stop/time-stop/friction backtest engine (real STT/exchange/SEBI/stamp/DP/GST + tax via `indiaFriction.ts`, NIFTY benchmark) over the F&O-eligible universe. Two of four named families implemented (delivery-surge, momentum-breakout); OI-buildup and FII-flow-tailwind explicitly deferred — their archives (~19d, ~1-2d) are too shallow to encode honestly. **No promotion, by design**: `npx tsx src/jobs/india-lab.ts <family>` is a manual research CLI (not scheduled — no live India genomes exist yet). Verified live: momentum-breakout produced 315 real trades, PF 0.64 (win rate 38%) — below the bar, honestly graveyarded (`india-momentum-breakout`, not hidden). *(Corrected from an initial PF 0.22 read: the first run backtested at `qty:1`, which turned the flat ₹15+GST DP charge into an outsized per-trade cost artifact at small notionals; fixed to size each trade at ~₹100k, matching the Treasury's real convention — the reject verdict didn't change, but the magnitude is now honest. A stale `id=7` graveyard row from the pre-fix run still sits alongside the corrected `id=8` — left in place pending the operator's own call on deleting it.)* Delivery-surge triggered 0 trades on the current archive depth — a data-sufficiency gap, not graveyarded.
- ✅ **Trade post-mortems** — `postmortem.ts`: every closed position auto-examined nightly (thesis right/lucky/wrong/unclear, did the exit follow the plan, which guards fired) — deterministic, not LLM-decided. Frozen to `trade_postmortems`, surfaced via `/postmortems`. Verified live against the one real closed position system-wide (correctly classified WRONG — loss, no plan-triggered guard event, since Watchtower only guards personal book holdings, not engine positions). Near-zero output is expected, not a gap — it accrues one record per closed trade going forward.

**Exit bar:** at least one India family clears the gauntlet (PF ≥ 1.3 net of India friction over 30 closed signals) and the INR book runs on its own Radar, benchmarked to NIFTY. **Not yet met** — see the India-native signal-family entry above: the machinery exists and reports honest in-sample numbers (momentum-breakout: PF 0.22 on 315 real trades, graveyarded), but no family has cleared a real out-of-sample walk-forward, and the archive isn't deep enough yet for one to mean anything. A2's five other organs (Radar, friction/NIFTY, Calendar, News Sentry, post-mortems) are live; validated India-native alpha remains open, honestly, same as it was for the US engine at this stage.
